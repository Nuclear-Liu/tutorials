...
如果已经注册多个重定向 URI ，如果仅注册了部分重定向 URI ，或者未注册任何重定向 URI ，客户端必须使用 `redirect_uri` 请求参数在授权请求中包含一个重定向 URI 。

当授权请求中包含重定向 URI 时，授权服务器必须将收到的值与 [RFC3986] 第 6 节中定义的至少一个已注册的重定向 URI （或 URI 组件）进行比较和匹配（如果有任何重定向 URI 已注册）。
如果客户机注册包含完整的重定向 URI ，授权服务器**必须**使用 [RFC3986] 第 6.2.1 节中定义的**简单字符串比较**来比较这两个 URI 。

##### 3.1.2.4. Invalid Endpoint 无效端点

如果授权请求因缺少、无效或不匹配的重定向 URI 而未通过验证，授权服务器**应将错误信息告知资源所有者**，且不得自动将用户代理重定向至无效的重定向 URI 。

##### 3.1.2.5. Endpoint Content 端点内容

客户端端点的重定向请求通常会产生 HTML 文档**响应**，**由用户代理处理**。
如果 HTML 响应是作为重定向请求的结果直接提供的，则 HTML 文档中包含的任何脚本都将在完全访问重定向 URI 及其包含的凭据的情况下执行。

客户端不应在重定向端点响应中包含任何第三方脚本（如第三方分析、社交插件、广告网络）。
相反，客户端应从 URI 中提取凭证，并将用户代理再次重定向到另一个端点，而不是暴露凭据（在 URI 或其他地方）。
如果包含第三方脚本，客户端必须确保首先执行自己的脚本（用于从 URI 中提取和删除凭据）。

### 3.2. Token Endpoints 令牌端点

令牌端点用于客户端通过提交**授权许可**或**刷新令牌**来获取获取**访问令牌**。
除了隐式授权类型（因为访问令牌是直接签发的）外，令牌端点与每个授权许可一起使用。

客户端获取令牌端点位置的方法超出了本规范的范围，但服务文档中通常会提供该位置。

端点 URI 可以包括 `application/x-www-form-urlencoded` 格式（根据附录 B）的查询组件（[RFC3986] 第 3.4 节），在添加其他查询参数时必须保留该组件。
缎带你 URI 不得包含片段组件。

**客户端发出访问令牌请求时，必须使用 HTTP `POST` 方法。**

发送的参数如果没有值，必须视为请求中缺省的参数。
授权服务器**必须忽略**无法识别的请求参数。
请求和响应参数不得包含多次。

#### 3.2.1. Client Authentication 客户端身份验证

当向令牌端点发出请求时，机密客户端或其他发出客户端凭证的客户端必须按照第 2.3 节中所述通过服务器进行身份验证。
客户端身份验证用于：

* 强制将**刷新令牌**和**授权码****绑定**到它们所发给的客户端。当授权代码通过不安全的信道传输到重定向端点或重定向 URI 尚未完成注册时，客户端身份验证至关重要。
* 通过禁用客户端或更改其凭据来恢复受损客户端，从而**防止攻击者滥用窃取到的刷新令牌**。**更改一套客户端凭证要比撤销整套刷新令牌快得多**。
* 实施身份验证管理最佳实践，要求**定期轮换凭证**。轮换整套刷新令牌很有难度，而轮换单套客户凭据则容易很多。

客户端在向令牌端点发送请求时可以使用 `client_id` 请求参数来**标识自己**。
在对令牌端点的 `authorization_code` `grant_type` 请求中，未经身份验证的客户端必须发送其 `client_id` ，以防止自己无意中接受针对具有不同 `client_id` 的客户端的代码。
这可以**保护客户端免遭身份验证代码的替换**。
（它不为受保护的资源提供额外的安全性。）

### 3.3. Access Token Scope 访问令牌范围

授权和令牌端点允许客户端使用 `scope` 请求参数指定访问请求的范围。
反过来，授权服务器也会使用 `scope` 响应参数来通知客户端所发出的访问令牌的范围。

范围参数的值以**空格**分隔、大小写敏感的字符串列表的形式表示。
这些字符串由授权服务器定义。
