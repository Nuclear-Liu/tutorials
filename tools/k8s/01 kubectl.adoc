...
使用 kubeasz 安装的 Kubernetes 集群默认所有节点( master 节点和 worker 节点)都可以使用 kubectl 对整个集群进行管理。

=== kubeadm 安装部署的集群

使用 kubeadm 安装的 Kubernetes 集群默认情况下，工作节点存在 kubectl 客户端工具，但是缺少连接集群的配置信息。

. 在 worker 节点创建配置文件目录: ``mkdir ~/.kube``

. 在 master 节点拷贝配置文件到 worker 节点配置文件目录: ``scp /etc/kebernetes/admin.conf __worker-node__:__USER_HOME__/.kube/config``

=== 二进制安装部署的集群

. 搜索 kubectl 客户端工具在 master 节点位置: ``whick kubectl``

. 在 master 节点将 kubectl 客户端工具拷贝到 worker 节点相同目录: ``scp /usr/local/bin/kubectl __worker-node__:/user/local/bin/kubectl``

. 在 worker 节点创建配置文件目录: ``mkdir ~/.kube``

. 在 master 节点拷贝配置文件到 worker 节点配置文件目录: ``scp __kubernetes-install-path__/kube.config __worker-node__:__USER_HOME__/.kube/config``


== 5. kubectl 最佳实践

include::action/node-manage.adoc[]

include::02 test.adoc[]

[source,java]
----
include::../../language/java/disruptor/src/main/java/org/hui/Main.java[]
----

[source,java]
----
include::../../language/java/disruptor/src/main/java/org/hui/Main.java[lines=6..11]
----